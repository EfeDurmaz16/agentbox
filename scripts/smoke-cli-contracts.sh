...
"${CLI[@]}" setup-plan --provider podman --json >"$TMPDIR/setup-plan-podman.json"
validate_json "$TMPDIR/setup-plan-podman.json" \
  "data.get('schema_version') == 1 and data.get('provider') == 'podman' and data.get('required_failed') == 0 and all(step.get('severity') == 'advisory' for step in data.get('steps', [])) and all(step.get('check') in ['podman CLI', 'podman machine', 'podman host bridge'] for step in data.get('steps', [])) and any(step.get('command') in ['install Podman', 'podman machine init && podman machine start', 'podman machine start', 'scripts/build-linux-shim.sh'] for step in data.get('steps', []))"
