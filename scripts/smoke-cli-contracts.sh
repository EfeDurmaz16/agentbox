...
validate_json "$TMPDIR/native-plan-linux-seccomp.json" \
  "data.get('provider') == 'agentpod-linux' and data.get('seccomp', {}).get('enabled') == True and data.get('seccomp', {}).get('syscall_rules', [])[0].get('syscall') == 'kill' and data.get('seccomp', {}).get('import_descriptor', {}).get('generated_oci_profile') == True and data.get('seccomp', {}).get('import_descriptor', {}).get('import_enabled') == False and 'external OCI/libseccomp profile import' in data.get('seccomp', {}).get('import_descriptor', {}).get('claim_boundary', '') and data.get('network_enforcement', {}).get('env_var') == 'AGENTBOX_LINUX_NETWORK_GUARD' and data.get('network_enforcement', {}).get('enabled') == False and 'descriptor only' in data.get('network_enforcement', {}).get('enforcement_claim', '') and data.get('nftables', {}).get('live_gate', {}).get('env_var') == 'AGENTBOX_LINUX_NFTABLES' and data.get('nftables', {}).get('live_gate', {}).get('enabled') == False and 'session nftables lifecycle' in data.get('nftables', {}).get('live_gate', {}).get('lifecycle_claim', '') and 'socket cgroupv2' in data.get('nftables', {}).get('session_scope', {}).get('cgroup_match', '') and data.get('nftables', {}).get('packet_policy', {}).get('cgroup_scoped') == True and 'agentpod.linux.runner.nftables.installed' in data.get('nftables', {}).get('lifecycle', {}).get('evidence_events', []) and 'agentpod.linux.runner.nftables.removed' in data.get('nftables', {}).get('lifecycle', {}).get('evidence_events', []) and 'agentpod.linux.runner.nftables.denied_packet' in data.get('nftables', {}).get('lifecycle', {}).get('evidence_events', []) and data.get('cgroup', {}).get('pids_max') == 64 and data.get('mount_namespace', {}).get('workspace_bind_mount_wired') == True and 'agentbox-linux-runner' in data.get('mount_namespace', {}).get('workspace_mount_claim', '') and 'runner-managed workspace mount' in data.get('security_claim', '') and data.get('mount_namespace', {}).get('overlayfs', {}).get('requires_overlayfs') == True and any(p.get('name') == 'bind-workspace' and p.get('status') == 'prototype' and p.get('evidence_event') == 'agentpod.linux.runner.workspace.mounted' for p in data.get('runner_phases', [])) and any(p.get('name') == 'apply-overlayfs' and p.get('status') == 'prototype' and p.get('evidence_event') == 'agentpod.linux.runner.overlayfs.applied' and 'mount overlayfs' in p.get('claim', '') for p in data.get('runner_phases', [])) and any(p.get('name') == 'apply-seccomp' and p.get('status') == 'prototype' and p.get('evidence_event') == 'agentpod.linux.runner.seccomp.applied' for p in data.get('runner_phases', [])) and any(p.get('name') == 'apply-network-guard' and p.get('status') == 'inactive' and p.get('evidence_event') == 'agentpod.linux.runner.network_guard.applied' for p in data.get('runner_phases', [])) and any(p.get('name') == 'apply-nftables' and p.get('status') == 'inactive' and p.get('evidence_event') == 'agentpod.linux.runner.nftables.installed' for p in data.get('runner_phases', []))"
validate_json "$TMPDIR/native-plan-linux-seccomp.json" \
  "data.get('lifecycle', {}).get('fail_closed') == True and data.get('lifecycle', {}).get('timeout', {}).get('kill_target') == 'cgroup-process-tree' and data.get('lifecycle', {}).get('timeout', {}).get('evidence_event') == 'agentpod.linux.lifecycle.timeout.kill_tree' and any(g.get('artifact') == 'runner-request-file' and g.get('evidence_event') == 'agentpod.linux.lifecycle.cleanup.request_file' for g in data.get('lifecycle', {}).get('cleanup_gates', [])) and any(g.get('artifact') == 'cgroup-v2-directory' and g.get('evidence_event') == 'agentpod.linux.lifecycle.cleanup.cgroup' for g in data.get('lifecycle', {}).get('cleanup_gates', [])) and any(g.get('artifact') == 'nftables-table' and g.get('evidence_event') == 'agentpod.linux.lifecycle.cleanup.nftables' for g in data.get('lifecycle', {}).get('cleanup_gates', [])) and any(e.get('event_name') == 'agentpod.linux.lifecycle.setup_failed' and e.get('cleanup_required') == True for e in data.get('lifecycle', {}).get('failure_events', []))"
validate_json "$TMPDIR/native-plan-linux-seccomp.json" \
...
validate_json "$TMPDIR/native-plan-linux-seccomp.json" \
  "data.get('ebpf', {}).get('collector', {}).get('env_var') == 'AGENTBOX_LINUX_EBPF' and data.get('ebpf', {}).get('collector', {}).get('enabled') == False and data.get('ebpf', {}).get('collector', {}).get('status') == 'unavailable' and data.get('ebpf', {}).get('collector', {}).get('unavailable_reason') == 'gate-disabled' and data.get('ebpf', {}).get('collector', {}).get('failure_mode') == 'skip-or-unavailable-not-pass' and data.get('ebpf', {}).get('collector', {}).get('event_status') == 'observed-only' and 'CAP_BPF' in data.get('ebpf', {}).get('collector', {}).get('required_privileges', []) and 'CAP_PERFMON' in data.get('ebpf', {}).get('collector', {}).get('required_privileges', []) and 'does not enforce policy' in data.get('ebpf', {}).get('collector', {}).get('claim_boundary', '')"
"${CLI[@]}" native-plan \
  --provider agentpod-linux \
  --workspace "$TMPDIR" \
...
validate_json "$TMPDIR/native-plan-macos.json" \
  "data.get('schema_version') == 1 and data.get('provider') == 'agentpod-macos' and data.get('virtualization', {}).get('requires_apple_virtualization') == True and data.get('virtualization', {}).get('linux_boot', {}).get('boot_loader') == 'VZLinuxBootLoader' and data.get('virtualization', {}).get('linux_boot', {}).get('kernel_image_host_path') is None and 'console=hvc0' in data.get('virtualization', {}).get('linux_boot', {}).get('kernel_command_line', []) and data.get('endpoint_security', {}).get('requires_system_extension') == True and data.get('network_extension', {}).get('requires_network_extension') == True and any(c.get('name') == 'vm-runner-binary' and c.get('required') == True and c.get('status') == 'planned' and 'agentbox-macos-vm-runner' in c.get('probe', '') for c in data.get('prerequisite_checks', [])) and any(c.get('name') == 'linux-kernel-image' and c.get('required') == True and 'AGENTBOX_MACOS_VM_KERNEL_IMAGE' in c.get('probe', '') for c in data.get('prerequisite_checks', [])) and any(c.get('name') == 'linux-initial-ramdisk' and c.get('required') == True and 'AGENTBOX_MACOS_VM_INITRD_IMAGE' in c.get('probe', '') for c in data.get('prerequisite_checks', [])) and any(p.get('name') == 'compile-vm-cell-config' and p.get('status') == 'descriptor' for p in data.get('runner_phases', [])) and any(p.get('name') == 'validate-vm-configuration' and p.get('status') == 'gated-prototype' for p in data.get('runner_phases', [])) and any(p.get('name') == 'start-virtualization-vm' and p.get('status') == 'gated-prototype' for p in data.get('runner_phases', [])) and any(p.get('name') == 'exec-command' and p.get('status') == 'planned' for p in data.get('runner_phases', [])) and data.get('live_env_var') == 'AGENTBOX_MACOS_NATIVE' and data.get('live_execution_enabled') == False and 'execution is not wired' in data.get('security_claim', '')"
validate_json "$TMPDIR/native-plan-macos.json" \
...
grep -q "Gated macOS AgentPod VM runner" "$TMPDIR/macos-vm-runner-help.txt"
grep -q "AGENTBOX_MACOS_VM_BOOT_PROTOTYPE=1" "$TMPDIR/macos-vm-runner-help.txt"
