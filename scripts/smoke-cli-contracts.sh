...
validate_json "$TMPDIR/native-plan-linux-seccomp.json" \
  "data.get('ebpf', {}).get('enforcement') == 'ObservedOnly' and len(data.get('ebpf', {}).get('receipts', [])) == len(data.get('ebpf', {}).get('event_sources', [])) and any(r.get('event_type') == 'linux.process.exec' and r.get('status') == 'descriptor-only-or-unobserved' and r.get('enforcement') == 'observed-only' and r.get('session_id') and r.get('correlation', {}).get('pid_fallback') == True and 'session_id' in r.get('process_identity_fields', []) and 'cgroup_path' in r.get('process_identity_fields', []) and 'not enforcement proof' in r.get('claim_boundary', '') for r in data.get('ebpf', {}).get('receipts', [])) and any(r.get('event_type') == 'linux.network.connect' and 'destination' in r.get('event_identity_fields', []) for r in data.get('ebpf', {}).get('receipts', []))"
