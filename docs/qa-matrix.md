...
| Linux native AgentPod | `agentpod-linux` is a gated `prototype primitive`. It can exercise native runner phases on suitable Linux hosts, but it is not a complete sandbox claim. | Portable CI for the conformance wrapper; optional/live for the gated native smoke. Live smoke is skipped by host dependency on non-Linux hosts or when `unshare`, `jq`, Landlock, writable/delegated cgroups v2, overlayfs, or the runner gate is missing. | `bash scripts/smoke-linux-agentpod-conformance.sh`<br>`AGENTBOX_LINUX_NATIVE=1 bash scripts/smoke-linux-native.sh`<br>`AGENTBOX_LINUX_NFTABLES=1 bash scripts/smoke-linux-nftables.sh` | Portable CI proves the clean-checkout native-plan contract and prints the exact live command. Live pass only for the specific prototype behavior exercised: namespace runner, read/write/execute Landlock allow/deny fixtures, cgroup attachment, seccomp-deny rules, coarse connect-deny fixture for deny-all network modes, overlay-review proof, eBPF observability receipt shape in the native plan, and nftables table lifecycle when separately gated. Do not claim eBPF probe loading/live capture, complete Landlock ABI coverage beyond modeled path-beneath rules, complete libseccomp import, domain allowlist enforcement, or packet firewall denial. |
