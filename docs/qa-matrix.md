...
| Linux native AgentPod | `agentpod-linux` is a gated `prototype primitive`. It can exercise native runner phases on suitable Linux hosts, but it is not a complete sandbox claim. Remaining gaps are mapped in [Linux hardening gaps](linux-hardening-gaps.md). | Portable CI for the conformance wrapper; optional/live for the gated native smoke. Live smoke is skipped by host dependency on non-Linux hosts or when `unshare`, `jq`, Landlock, writable/delegated cgroups v2, overlayfs, nftables privileges, or the runner gate is missing. | `bash scripts/smoke-linux-agentpod-conformance.sh`<br>`AGENTBOX_LINUX_NATIVE=1 bash scripts/smoke-linux-native.sh`<br>`AGENTBOX_LINUX_NFTABLES=1 bash scripts/smoke-linux-nftables.sh`<br>`AGENTBOX_LINUX_EBPF=1 cargo test --locked -p agentbox-daemon linux_ebpf` | Portable CI proves the clean-checkout native-plan contract and prints the exact live command. Live pass only for the specific prototype behavior exercised: namespace runner, mount boundary contract for host-root/private mount namespace/procfs/tmp/dev semantics, sensitive host path and device-node unavailable-or-mediated probes, no leaked `/workspace` host mount, ABI-aware Landlock read/write/create/remove/execute plus host-supported `REFER`/`TRUNCATE` allow/deny fixtures, cgroup attachment and cleanup, timeout process-tree kill through the session cgroup, generated seccomp-deny rules, imported OCI/libseccomp subset deny rules, coarse connect-deny fixture for deny-all network modes, session-cgroup-scoped nftables IP/CIDR packet denial and cleanup when separately gated, overlay-review proof, request-file cleanup, eBPF observability receipt shape, and gated observed-only eBPF collector/evidence export unavailable semantics. Do not claim eBPF probe loading/live kernel capture, hardened rootfs isolation, private tmpfs, private device namespace, device `ioctl` mediation, complete Landlock ABI coverage beyond modeled path-beneath rules, complete libseccomp compatibility beyond the supported import subset, live domain allowlist enforcement, DNS semantic completeness, or complete packet firewall coverage. |
