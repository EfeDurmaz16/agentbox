...
agentbox run --provider agentpod-linux --seccomp-profile ./seccomp.json --plan -- /bin/true
agentbox minipod-spec hermes --provider agentpod-linux --deny-syscall kill --max-processes 64
agentbox minipod-spec hermes --provider agentpod-linux --seccomp-profile ./seccomp.json
agentbox native-plan --provider agentpod-linux --deny-syscall kill --max-processes 64 -- /bin/true
agentbox native-plan --provider agentpod-linux --seccomp-profile ./seccomp.json -- /bin/true
```

The Linux native plan can now import a narrow OCI/libseccomp JSON subset through
`--seccomp-profile`. The importer accepts `defaultAction: SCMP_ACT_ALLOW`, the
current host architecture, and unconditional syscall rules using
`SCMP_ACT_ERRNO` or `SCMP_ACT_KILL_PROCESS` for syscalls supported by the
prototype BPF loader. Unsupported default-deny profiles, argument-conditional
rules, `includes`/`excludes`, notify/listener fields, loader flags, unknown
fields, unsupported architectures, unsupported syscalls, and unsupported actions
fail closed with actionable errors. This is still not complete libseccomp
compatibility.
...
The VM cell boot request now carries the Apple-documented Linux boot artifacts:
`VZLinuxBootLoader`, a kernel image path from `AGENTBOX_MACOS_VM_KERNEL_IMAGE`,
an initial RAM disk path from `AGENTBOX_MACOS_VM_INITRD_IMAGE`, and a
`console=hvc0` command line. The daemon also defines the typed macOS VM runner
request payload and ships `agentbox-macos-vm-runner`. Without
`AGENTBOX_MACOS_VM_BOOT_PROTOTYPE=1`, the runner only validates request JSON and
refuses execution. With that gate, it emits a typed boot-prerequisite report and
can attempt the Apple Virtualization path only when the host, boot artifacts,
Swift toolchain, `codesign`, and `com.apple.security.virtualization`
prerequisites are present. `AGENTBOX_MACOS_NATIVE=1` remains limited to
native-plan and runner-request experiments; it does not make the
`agentpod-macos` runtime provider available or authorize `agentbox run`.
...
Provider execution remains unavailable until VM boot, host bridge evidence,
cleanup, a signed system extension, Network Extension lifecycle, and live
allow/deny evidence tests are wired together.
See [macOS Apple Virtualization boot prototype](macos-virtualization-boot.md)
for the gated boot path and Apple prerequisite boundary.
