...
contract-only `agentbox-macos-vm-runner` binary. `AGENTBOX_MACOS_NATIVE=1`
is limited to native-plan and runner-request experiments; it does not make the
...
Provider execution remains unavailable until the VM runner can boot cells,
a signed system extension, Network Extension lifecycle, and live allow/deny
