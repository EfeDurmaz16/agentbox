...
| Kernel event evidence | eBPF observability receipt schemas in the native plan, including provider, session, cgroup path, event identity, pid/tgid fallback fields, observed-only semantics, a gated `AGENTBOX_LINUX_EBPF=1` collector contract, unavailable-by-default host probing, and redacted observed-event evidence export. | #264 | Do not claim live eBPF probe loading, live kernel capture, enforcement, or observed events as denial proof. |
