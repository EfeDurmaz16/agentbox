...
| `podman-compat` | Podman compatibility backend for container-backed sessions when the target host passes live smoke. Legacy `podman` is a deprecated CLI alias. | Medium to high when host mounts, daemon sockets, or VM layers are treated as Agentbox-owned native enforcement. | Operator check: run `bash scripts/smoke-podman-bridge.sh` on the target host. Mitigation: treat it as compatibility only. Non-goal: paid-product isolation center or uniform cross-platform enforcement. |
| `agentpod-linux` | Gated native prototype with Linux namespace, cgroup, Landlock, seccomp, overlayfs, and runner prerequisites. | High until denial, cleanup, workspace, credential, network, and evidence gates pass live on Linux. | Operator check: run `AGENTBOX_LINUX_NATIVE=1 bash scripts/smoke-linux-native.sh` on Linux. Mitigation: use only for prototype validation. See [Linux hardening gaps](linux-hardening-gaps.md). Non-goal: default paid-product backend or complete sandbox. |
| `agentpod-macos` | Descriptor plus gated Apple Virtualization boot prototype; native provider execution remains unavailable until live VM, entitlement, denial, evidence, and cleanup gates pass. | High if boot-prerequisite or descriptor reports are read as shipped macOS enforcement. | Operator check: inspect provider truth and future macOS live smoke results before any public claim. `AGENTBOX_MACOS_VM_BOOT_PROTOTYPE=1` does not enable `agentbox run --provider agentpod-macos`. Non-goal: native macOS AgentPod execution or enforcement today. |
