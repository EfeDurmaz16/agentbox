...
| `agentpod-linux` | Gated native prototype on Linux hosts that satisfy namespace, cgroup v2, Landlock, seccomp, overlayfs, nftables, and runner prerequisites. | Default paid-product backend, complete sandbox, hardened rootfs isolation, private tmpfs, private device namespace, device `ioctl` mediation, live eBPF probe loading/kernel capture, complete Landlock ABI coverage beyond modeled path-beneath rules, complete libseccomp compatibility beyond the supported import subset, live domain allowlist enforcement, DNS semantic completeness, or complete packet firewall coverage. The issue-mapped boundary is in [Linux hardening gaps](linux-hardening-gaps.md). | `bash scripts/smoke-linux-agentpod-conformance.sh` runs in CI and verifies the clean-checkout native-plan contract. `AGENTBOX_LINUX_NATIVE=1 bash scripts/smoke-linux-native.sh` is the live Linux gate. The portable native-plan contract proves eBPF receipts and gated observed-only collector/evidence export identify session/process fields without claiming enforcement; unit and live fixtures prove the mount boundary contract, unavailable-or-mediated host sensitive path and device node probes, generated and imported seccomp `kill(2)` denial for the supported subset, Landlock ABI-aware supported subset including host-supported `REFER`/`TRUNCATE`, lifecycle request/cgroup/nftables cleanup events, and timeout cgroup process-tree kill policy; the Linux-only unit fixture proves coarse `connect(2)` denial for deny-all network modes; `AGENTBOX_LINUX_NFTABLES=1 bash scripts/smoke-linux-nftables.sh` proves session-cgroup-scoped nftables IP/CIDR packet denial and cleanup, while domain policy remains resolver/ipset-gated. |
| `agentpod-macos` | Descriptor and gated Apple Virtualization boot prototype until live VM lifecycle, signed system extension, Network Extension, and denial tests exist. | Native macOS AgentPod execution or enforcement. `AGENTBOX_MACOS_VM_BOOT_PROTOTYPE=1` is only a boot-prerequisite/start path and does not enable provider execution. | Current release gates may verify provider truth, the Linux boot artifact contract, and compatibility smoke, but they do not prove native macOS execution. A future paid claim needs a live macOS native smoke that boots, mediates, denies, records evidence, and cleans up. |
