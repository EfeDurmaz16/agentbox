...
| Remote AgentPod | Descriptor only | The `remote-agentpod` descriptor models remote/disposable worker execution over a remote bridge. `agentbox remote-descriptor` emits a secret-free transport/auth/evidence/lifecycle descriptor with required kill-switch acknowledgement, but transport handshake, worker lifecycle, command execution, evidence streaming, and kill-switch enforcement are not implemented. See [Remote AgentPod](remote-agentpod.md). |
