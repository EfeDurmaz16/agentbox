...
| Remote AgentPod | Descriptor only | The `remote-agentpod` descriptor models remote/disposable worker execution over a remote bridge. `agentbox remote-descriptor` emits a secret-free transport/auth/evidence/lifecycle descriptor and `agentbox remote-handshake` emits a secret-free signed-challenge descriptor, but transport handshake verification, worker lifecycle, command execution, evidence streaming, and kill-switch enforcement are not implemented. See [Remote AgentPod](remote-agentpod.md). |
