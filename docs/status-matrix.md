...
| Setup command | Shipped partial | `agentbox setup` initializes local config and direct-host command shims. `agentbox setup --dry-run --json` emits a machine-readable setup report without mutating host state, and `--provider ...` scopes setup reporting to one backend. Provider-specific automatic installers are not implemented yet. |
