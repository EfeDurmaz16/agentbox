...
| Doctor command | Shipped | `agentbox doctor` reports daemon, shim, audit, PATH, provider readiness, remote AgentPod endpoint readiness, macOS native planning prerequisites such as Apple Virtualization plus future ES/NE entitlements, Linux native prototype prerequisites such as user namespaces, cgroups v2, seccomp, and Landlock ABI availability, and Windows native/VM-backed gaps such as Job Objects, AppContainer, WFP, ETW, and Sandbox/Hyper-V readiness. `agentbox doctor --json` emits the same checks with `required_failed` versus `advisory_failed` counts so installers and release gates block on broken shipped setup without treating planned provider prerequisites as shipped requirements. |
| Setup plan command | Shipped | `agentbox setup-plan` and `agentbox setup-plan --json` derive the next local operator actions from the doctor report without mutating host state. `--provider direct-host|podman|agentpod-macos|agentpod-linux|agentpod-windows|remote-agentpod` narrows the plan to one backend. Steps carry required/advisory severity, release-blocker truth, human action text, and a best-effort command when one is safe to suggest. |
| Setup command | Shipped partial | `agentbox setup` initializes local config and direct-host command shims. `agentbox setup --dry-run --json` emits a machine-readable setup report without mutating host state, including deduplicated `operator_commands` from the readiness plan, and `--provider ...` scopes setup reporting to one backend. Provider-specific automatic installers are not implemented yet. |
...
| Runtime provider listing | Shipped | `agentbox providers` reports family, platform, shipped/experimental/descriptor-only status, and network enforcement claims. `agentbox providers --json` exposes the same provider truth metadata plus boundary primitive labels, setup commands, doctor check names, and verification commands for scripts, installers, and release checks. |
...
| AgentPod provider truth | Shipped | `agentpod-macos`, `agentpod-linux`, `agentpod-windows`, and `remote-agentpod` expose capability/status metadata. `agentbox providers --json` also reports planned boundary primitives such as Apple Virtualization, Linux namespaces/seccomp/Landlock, and Windows Job Objects/WFP/ETW without claiming they are active enforcement. Linux reports prototype primitives; macOS and Windows remain descriptor-only; remote is experimental and only becomes available when `AGENTBOX_REMOTE_AGENTPOD_ENDPOINT` points at an HTTPS worker, or at loopback HTTP with `AGENTBOX_REMOTE_AGENTPOD_ALLOW_HTTP_LOOPBACK=1` for local verification. |
