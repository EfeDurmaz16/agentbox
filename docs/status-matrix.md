...
| Setup plan command | Shipped | `agentbox setup-plan` and `agentbox setup-plan --json` derive the next local operator actions from the doctor report without mutating host state. Steps carry required/advisory severity, release-blocker truth, human action text, and a best-effort command when one is safe to suggest. |
