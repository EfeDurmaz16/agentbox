...
| Remote AgentPod | Descriptor only | The `remote-agentpod` descriptor models remote/disposable worker execution over a remote bridge. `agentbox remote-descriptor` emits a secret-free transport/auth/evidence/lifecycle descriptor and `agentbox remote-handshake` emits a secret-free signed-challenge descriptor. The daemon also has an in-memory transport conformance contract plus an HTTPS adapter skeleton for handshake/create/exec/destroy response validation, including canonical challenge binding and required destroy-time `KillSwitchAck` when the kill switch is required. Provider wiring, a shipped worker server, cryptographic signed response verification, evidence streaming, and live kill-switch enforcement are not implemented. See [Remote AgentPod](remote-agentpod.md). |
